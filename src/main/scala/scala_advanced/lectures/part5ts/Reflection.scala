package scala_advanced.lectures.part5ts
...
object Reflection extends App {
  ...
  val clazz = m.staticClass("scala_advanced.lectures.part5ts.Reflection.Person") // creating a class object by NAME
  ...
  // method name computed from somewhere elsef
...
//  def processList(list: List[String]): Int = 45
  ...
  def getTypeArguments[T](value: T)(implicit typeTag: TypeTag[T]) =
    typeTag.tpe match {
  ...
  def isSubtype[A, B](implicit ttagA: TypeTag[A], ttaB: TypeTag[B]): Boolean = {
    ttagA.tpe <:< ttaB.tpe
  }
