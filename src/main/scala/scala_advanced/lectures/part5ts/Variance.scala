...
//  }
  ...
  // return types
  class PetShop[-T] {
//    def get(isItaPuppy: Boolean): T // METHOD RETURNS TYPES ARE IN COVARIANT POSITION
    /*
...
//  val evilCat = shop.get(true, new Cat)
  ...
  class CParking[+T](vehicles: List[T]) {
    def park[S >: T](vehicle: S): CParking[S] = ??? // S = super type
  ...
  class CParking2[+T](vehicles: IList[T]) {
    def park[S >: T](vehicle: S): CParking2[S] = ??? // S = super type
